	...
	private void definirEventos() {
					...
					if (tfPlaca.getText().isBlank()) {
						...
						if (dados != null) {
							...
							if (cor.equals("amarelo")) {
								liCor.setSelectedIndex((0));
							}
							else if (cor.equals("azul")) {
								liCor.setSelectedIndex((1));
							}
							else if (cor.equals("branco")) {
								liCor.setSelectedIndex((2));
							}
							else if (cor.equals("vermelho")) {
								liCor.setSelectedIndex((3));
							}
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					if (tfPlaca.getText().isBlank() || tfDescricao.getText().isBlank() || liCor.isSelectionEmpty()) {
						...
						String cor = liCor.getSelectedValue();
						JOptionPane.showMessageDialog(null, carro.alteraCarro(tfPlaca.getText(), cor, tfDescricao.getText()));
					    tfPlaca.setText("");
					    tfDescricao.setText("");
					    lbImagem.setIcon(null);
					    liCor.clearSelection();
		...
		btApaga.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					if (tfPlaca.getText().isBlank()) {
						JOptionPane.showMessageDialog(null, "Preencha s placa");
						tfPlaca.requestFocus();
					} else {
